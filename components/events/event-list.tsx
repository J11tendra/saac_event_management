...
import { Calendar, Clock, IndianRupee, ChevronDown } from "lucide-react";
          ...
          <CardContent className="p-4">
            <div className="space-y-3">
              ...
              <div className="flex flex-wrap gap-4 text-sm text-muted-foreground">
                {/* Budget */}
                {event.budget_request && event.budget_request.length > 0 && (
                  <div className="flex items-center gap-1">
                    <IndianRupee className="w-4 h-4" />
                    <span>
                      ₹
                      {event.budget_request[0].budget_amt.toLocaleString(
                        "en-IN"
                      )}
                      {event.budget_request[0].approved_budget && (
                        <span className="text-chart-2 font-semibold ml-1">
                          (✓ ₹
                          {event.budget_request[0].approved_budget.toLocaleString(
                            "en-IN"
                          )}
                          )
                        </span>
